...
# =====================================================================================
#  1. DEFINIÇÃO DAS VARIÁVEIS DO UNIVERSO FUZZY (ENTRADAS E SAÍDAS)
# =====================================================================================
# As 'Antecedents' são as entradas do nosso sistema: o que ele usa para pensar.
# O 'Consequent' é a saída do sistema: a decisão que ele toma.

# ENTRADA 1: Erro de Umidade
# Mede a diferença entre a umidade ideal e a umidade atual.
# Universo: de -50% a 50% de erro. Ex: -0.1 significa que está 10% mais úmido que o ideal.
erro_umidade = ctrl.Antecedent(np.arange(-0.5, 0.51, 0.01), 'erro_umidade')

# ENTRADA 2: Fator Clima
# Mede o quão "agressivo" o clima está para a evaporação.
# Universo: de 0 (clima perfeito, sem perdas) a 10 (calor extremo, ventania, sol forte).
fator_clima = ctrl.Antecedent(np.arange(0, 10.1, 0.1), 'fator_clima')

# SAÍDA: Vazão de Água
# A ação de controle que queremos tomar.
# Universo: de 0% a 100% da capacidade máxima da bomba.
vazao_agua = ctrl.Consequent(np.arange(0, 101, 1), 'vazao_agua')


# =====================================================================================
#  2. DEFINIÇÃO DAS FUNÇÕES DE PERTINÊNCIA (OS TERMOS LINGUÍSTICOS)
# =====================================================================================
# Aqui, nós definimos o que cada termo como "SECO", "IDEAL", "ÚMIDO" significa.
# Usamos formas geométricas (triângulos, trapézios) para mapear valores numéricos a conceitos fuzzy.

# Funções de pertinência para 'erro_umidade'
# O erro é (ideal - atual). Um erro positivo significa que o solo está SECO.
...
# Funções de pertinência para 'fator_clima'
...
# Funções de pertinência para 'vazao_agua'
...
# =====================================================================================
#  3. BASE DE REGRAS (O "CÉREBRO" DO CONTROLADOR)
# =====================================================================================
# Aqui está a lógica. Conectamos as entradas com as saídas usando regras "SE ... E ... ENTÃO ...".
...
# =====================================================================================
#  4. MONTAGEM E SIMULAÇÃO DO SISTEMA DE CONTROLE
# =====================================================================================
# Juntamos as regras e criamos o objeto de simulação.

# Criação do sistema de controle com as regras
sistema_controle = ctrl.ControlSystem([rule1, rule2, rule3, rule4, rule5, rule6, rule7, rule8, rule9, rule10, rule11])

# Criação da simulação que efetivamente vai calcular os resultados
simulador_pivo = ctrl.ControlSystemSimulation(sistema_controle)


# =====================================================================================
#  5. FUNÇÃO PRINCIPAL PARA SER CHAMADA PELO SEU MODELO
# =====================================================================================
# Esta é a única função que você vai precisar chamar do seu arquivo modelo.py.
...
def get_controle_fuzzy(valor_erro_umidade, valor_fator_clima):
    ...
    # Executa a lógica Fuzzy para calcular o resultado
    simulador_pivo.compute()

    # Retorna o valor de saída "crisp" (desnebulizado)
...
def get_controle_motor_fuzzy(declive_abs, torque_motor, torque_motor_max):
    ...
    # Funções de pertinência para declive (3° é o limite crítico)
    ...
    # Funções de pertinência para torque percentual (mantidas iguais)
    ...
    # Funções de pertinência para ativação do motor (mantidas iguais)
    ...
    # Regras fuzzy (declive > 3° tem prioridade absoluta)
